...
function AddToCartOnUpSell(formId, button) {
    var is_view = $jq('#product_addtocart_form').attr('method');
    var effect_to_cart = $jq('.effect_to_cart').attr('value');

    if(is_view) {
                 url = $jq('#'+formId).attr('action');
                var data = $jq('#'+formId).serialize();
                //fly to basket
                
                var img = $jq('#'+formId).find('.product-img-box .product-image img');
                if(effect_to_cart==1) {
                    if($jq('#ajax_cart_super_product_view').attr('class')!='popup') {
                        flyToCart($jq(img), $jq('.top-link-cart'));
                    }
                ...
                var fileInput = $jq('#'+formId+' input[type="file"]');
                if(fileInput.length>0) {
                    var file = fileInput[0].files[0];
                }
...
function showBoxInfo(product_info,related_pro) {
    var base_url = $jq('#ajaxconfig_info a').attr('href');
...
function changeDelelteUrl() {
    var str =null /*'<script type ="text/javascript">\n\
        ...
        $jq.ajax({
            ...
            success: function(data){
                if(data.status==1) {
					...
					if(data.shopping_bag_link)
					{
						 $jq('.shopping-bag-link').html('');
                        $jq('.shopping-bag-link').append(data.shopping_bag_link);
					}
                   ...
                   console.log(data.related_pro);
					 //console.log(data.product_and_cart);
                    ...
                    if(data.product_info) {
                        ...
                        showBoxInfo(data.product_info,data.related_pro);
                    }else {
                        hideLoadingAnimation();
                    }
...
function deleteCartInCheckoutPage(){  //return true;
...
//    })
    $jq('#continue_shopping, #shopping_cart').live('click', function(){
	   ...
	   $jq('#close_popup').live('click',function(){
