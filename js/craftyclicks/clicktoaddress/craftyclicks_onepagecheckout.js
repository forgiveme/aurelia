...
//				var tmp_html = '<li class="'+this.li_class+'" style="display: none"><div class="one-field" id="'+this.prefix+'_cp_result_display">&nbsp;</div></li>';
			...
			if (!$(this.prefix+'_cp_button_div_id')) {
				...
				if ('' != _cp_button_image) {
					...
					tmp_html += '&nbsp;<img style="cursor: pointer;" src="'+_cp_button_image+'" id="'+this.prefix+'_cp_button_id_enter_manually" class="'+_cp_button_class+'" title="'+_cp_button_text_enter_manually+'"/>';
				} else {
					...
					tmp_html += '<button type="button" id="'+this.prefix+'_cp_button_id_enter_manually" class="'+_cp_button_class+'"><span><span>'+_cp_button_text_enter_manually+'</span></span></button>';
				}
				if (0 == this.div_depth) {
					tmp_html += '</div>';							
				} else {
				...
				$(this.prefix+"_cp_button_id_enter_manually").observe('click', this.button_clicked_manual.bindAsEventListener(this));
			}
			// show button 
	...
	this.setup_for_non_uk = function() {
		...
		if(this.prefix == 'billing') {
			jQuery("#billing_address_list .hidden_default").show();
		} else {
			jQuery("#shipping_address_list .hidden_default").show();
		}
	}
	this.clear_fields = function(setup) {
		...
		if($(this.fields.company_id))
			$(this.fields.company_id).setValue("");
		if($(this.fields.street1_id))
			$(this.fields.street1_id).setValue("");
		if($(this.fields.street2_id))
			$(this.fields.street2_id).setValue("");
		if($(this.fields.street3_id))
			$(this.fields.street3_id).setValue("");
		if($(this.fields.street4_id))
			$(this.fields.street4_id).setValue("");
		if($(this.fields.town_id))
			$(this.fields.town_id).setValue("");
		if($(this.fields.county_id))
			$(this.fields.county_id).setValue("");
		if($(this.fields.postcode_id))
			$(this.fields.postcode_id).setValue("");
...
//			alert ('Postcode Lookup could not be added!');
	...
	this.button_clicked_manual = function(e) {
		if ('' != _cp_error_class) $(this.prefix+'_cp_result_display').removeClassName(_cp_error_class);
		if(this.prefix == 'billing') {
	...
	this.result_selected = function() {
		...
		$$('.'+this.prefix+'_cp_address_class').invoke('show');
		if(this.prefix == 'billing') {
			jQuery("#billing_address .hidden_default").show();
		} else {
			jQuery("#shipping_address .hidden_default").show();
		}
