...
(function($) {
	...
	var Drum = function(element, options, transformProp)
	{
		...
		settings.theta = 180 / settings.panelCount;
		...
		settings.radius = Math.round( ( $(drum).height() / 1.5 ) / Math.tan( Math.PI / settings.panelCount ) );
		settings.mapping = [];
		var c = 0;
		...
		var transform = function() {
			...
			if (selected) {
				...
				$(selected.elem).css({
						'opacity': '1',
						'color': 'rgb(182,113,154)',
						'font-weight': 'bold'
				});
				$("figure:not(.a" + (selected.angle*100) + ", .hidden)", drum).css({
																					'opacity': '0.5',
