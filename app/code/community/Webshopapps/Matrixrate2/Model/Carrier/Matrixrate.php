...
class Webshopapps_Matrixrate2_Model_Carrier_Matrixrate
    extends Mage_Shipping_Model_Carrier_Abstract
    implements Mage_Shipping_Model_Carrier_Interface
{

    protected $_code = 'matrixrate2';
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if ($freeShipping)
        {
			...
			$method->setCarrier('matrixrate2');
			$method->setCarrierTitle($this->getConfigData('title'));
			$method->setMethod('matrixrate2_free');
	   ...
	   foreach ($ratearray as $rate)
		{
		   if (!empty($rate) && $rate['price'] >= 0) {
			  $method = Mage::getModel('shipping/rate_result_method');
				...
				$method->setMethod('matrixrate2_'.$rate['pk']);

				$method->setMethodTitle(Mage::helper('matrixrate2')->__($rate['delivery_type']));
    ...
    {
        return Mage::getResourceModel('matrixrate2_shipping/carrier_matrixrate')->getNewRate($request,$this->getConfigFlag('zip_range'));
    }
    ...
    {
        return array('matrixrate2'=>$this->getConfigData('name'));
    }
