...
*/
class Webshopapps_Matrixrate2_Model_Mysql4_Carrier_Matrixrate extends Mage_Core_Model_Mysql4_Abstract
{
    protected function _construct()
    {
        $this->_init('shipping/matrixrate2', 'pk');
    }
    ...
    public function getNewRate(Mage_Shipping_Model_Rate_Request $request,$zipRangeSet=0)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTableName('matrixrate2_shipping/matrixrate');
		...
		for ($j=0;$j<10;$j++)
		{
			...
			{
			  $select->where('logged_in=?','1');
			}
			else
			{
			  $select->where('logged_in=?','0');
			}
			switch($j) {
				case 0:
					$select->where(
    ...
    {
        $csvFile = $_FILES["groups"]["tmp_name"]["matrixrate2"]["fields"]["import"]["value"];
        ...
        if (!empty($csvFile)) {
            ...
            */
            ...
            if (isset($_POST['groups']['matrixrate2']['fields']['condition_name']['inherit'])) {
                $conditionName = (string)Mage::getConfig()->getNode('default/carriers/matrixrate2/condition_name');
            } else {
                $conditionName = $_POST['groups']['matrixrate2']['fields']['condition_name']['value'];
            }
...
//            } else {
//                $conditionName = $websiteModel->getConfig('carriers/matrixrate2/condition_name');
//            }
            $conditionFullName = Mage::getModel('matrixrate2_shipping/carrier_matrixrate')->getCode('condition_name_short', $conditionName);
            if (!empty($csv)) {
                $exceptions = array();
                $csvLines = explode("\n", $csv);
                ...
                if (empty($exceptions)) {
                    ...
                    foreach ($csvLines as $k=>$csvLine) {
						...
						if (count($csvLine)==10) {
							...
							$data[] = array('website_id'=>$websiteId, 'dest_country_id'=>$countryId, 'dest_region_id'=>$regionId, 'dest_city'=>$city, 'dest_zip'=>$zip, 'dest_zip_to'=>$zip_to, 'condition_name'=>$conditionName, 'condition_from_value'=>$csvLine[5],'condition_to_value'=>$csvLine[6], 'price'=>$csvLine[7], 'delivery_type'=>$csvLine[8], 'logged_in'=>$csvLine[9]);
							...
							$data[] = array('website_id'=>$websiteId, 'dest_country_id'=>$countryId, 'dest_region_id'=>$regionId,  'dest_city'=>$city,'dest_zip'=>$zip,'dest_zip_to'=>$zip_to,  'condition_name'=>$conditionName, 'condition_from_value'=>$csvLine[3],'condition_to_value'=>$csvLine[4], 'price'=>$csvLine[5], 'delivery_type'=>$csvLine[6], 'logged_in'=>$csvLine[7]);
						}
