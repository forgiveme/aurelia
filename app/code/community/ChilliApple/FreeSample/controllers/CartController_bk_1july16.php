    ...
    public function addAction() {
		...
		try {  $product=$this->_initProduct();
    	  ...
    	  if($product->getSample())
    	  {
            ...
            if($sample && (count($samplesInCart)==$noOfProducts) && $sample->getId()!=$product->getId())
            {
               $msg=$this->__('You have already selected this sample, please discover something new!');
               if($noOfProducts>1)
               {
                 $msg=$this->__('You have already selected your %s complimentary samples.',$noOfProducts);
               }
    	   ...
    	   // $isAlloweMultipleUnits=Mage::getStoreConfig('promotion/sample/allow_multiple_units');
    	    ...
    	    if($sample && $sample->getId()==$product->getId())
    	    {
    	   	$this->_getSession()->addError($this->__('You have already selected this sample, please discover something new!'));
    	   	$this->_goBack();
	        return ;
			...
			if ($stockStatus==0):
			  $this->_goBack();
				...
				/*if($sample && !$isAlloweMultiple && $sample->getId()!=$product->getId())
				{
				 ...
				 // not more than X product(s) is/are allowed
				if($sample && (count($samplesInCart)==$noOfProducts) && $sample->getId()!=$samplePro)
				{
				   $msg=$this->__('You have already selected this sample, please discover something new!');
				   if($noOfProducts>1)
				   {
			   ...
			   // $isAlloweMultipleUnits=Mage::getStoreConfig('promotion/sample/allow_multiple_units');
				...
				//same product sample re-added
				if($sample && $sample->getId()==$samplePro)
				{
				$this->_getSession()->addError($this->__('You have already selected this sample, please discover something new!'));
				$this->_goBack();
				return ;
                ...
                foreach ($cartData as $index => $data) {
                    if (isset($data['qty'])) {
                        $cartData[$index]['qty'] = $filter->filter(trim($data['qty']));
                         ...
                         if(isset($sampleProducts[$index]) && $cartData[$index]['qty']>1)
                         {
                            $errors=true;
                         }
