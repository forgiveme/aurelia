...
class Cnc_MarketPlace_Block_Adminhtml_Orders extends Mage_Adminhtml_Block_Template
{
	...
	public function getOrderDetails()
	{
		...
		$order_acceptance        = $this->helper->getConfigurationData( 'order_acceptance' );
	...
	public function saveMirakleOrders( $offset, $max, $recursive )
	{
		...
		$automation = $this->helper->getConfigurationData( 'order_acceptance' );
		if ( $automation ) {
		...
		if ( $morders[ 'order_state' ] == 'WAITING_ACCEPTANCE' || $morders[ 'order_state' ] == 'SHIPPING' ) {
			...
			if ( trim( array_search( "", $is_there_a_product ) ) == '' && trim( array_search( "0", $is_prod_in_stylecom ) ) == '' && trim( array_search( "", $is_prod_in_stylecom ) ) == '' && trim( array_search( '0', $is_product_instock ) ) == '' ) {
				...
				if ( trim( $check_order_id[ 'm_order_id' ] ) == "" && $morders[ 'order_state' ] == 'SHIPPING' ) {
					$new_order_id     = $this->createMagentoOrder( $morders[ 'order_lines' ], $customer_details[ 'email' ], $BilladdressData, $ShipaddressData );
	...
	public function getFinalStatus( $lists, $mirakle_status )
	{
		foreach ( $lists as $list ) {
			if ( isset( $list->$mirakle_status ) && isset( $mirakle_status ) ) {
				$mgnto_value[ ] = $list->$mirakle_status;
			}
		}
		$magento_value = array_filter( $mgnto_value );
		sort( $magento_value );
		return $magento_value[ 0 ];
	}
	...
	public function saveNewMagentoOrderFromMirakle( $new_order_id, $final_status, $shippingprice, $mirakl_order_id )
	{
		...
		if ( trim( $shippingprice ) != '' ) {
			...
			$order_real->setBaseGrandTotal( $price + $shippingprice );
	...
	{
		$lists            = json_decode( $this->helper->getConfigurationData( 'status_mapping' ) );
	...
	public function createMagentoOrder( $order_lines, $customer_email, $BilladdressData, $ShipaddressData )
	{
		...
		foreach ( $order_lines as $mproducts ) {
			...
			$prod_sku = $mproducts[ 'product_sku' ];
		...
		$billingAddress  = $cart->getBillingAddress()->addData( $BilladdressData );
	...
	public function getValidatedCustomerDetails( $mcustomer )
	{
		...
		$valid_cust[ 'email' ]      = isset( $mcustomer[ 'customer_id' ] ) ? $mcustomer[ 'customer_id' ] : 'condenast@condenast.com';
		...
		$valid_cust[ 'bstreet2' ]   = isset( $billing_address[ 'street_2' ] ) ? $billing_address[ 'street_2' ] : 'CN Street';
		...
		$valid_cust[ 'bcountry' ]   = isset( $billing_address[ 'country' ] ) ? $billing_address[ 'country' ] : 'CN country';
		...
		$valid_cust[ 'scountry' ]   = isset( $shipping_address[ 'country' ] ) ? $shipping_address[ 'country' ] : 'CN country';
