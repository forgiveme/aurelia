			...
			if ( $mappedstate->track_state_mapping == $final_state ) {
				...
				foreach ( $shipmentCollection as $shipment ) {
					...
					foreach ( $shipment->getAllTracks() as $track ) {
						...
						$carrier_mapped   = json_decode( $helper->getConfigurationData( 'carrier_mapping' ) );
						$tracking_code    = array();
						foreach ( $carrier_mapped as $carrier_map ) {
							if ( isset( $carrier_map->$tracking_details[ 'carrier_code' ] ) && $carrier_map->$tracking_details[ 'carrier_code' ] )
								$tracking_code[ ] = $carrier_map->$tracking_details[ 'carrier_code' ];
						}
						...
						if ( $tracking_label[ 0 ] && isset( $tracking_label[ 0 ] ) && $tracking_details[ 'track_number' ] ) {
							$fields_selected = array(
								 'orderId' => $mirakle_data[ 'orderid' ],
								'carriercode' => $tracking_code[ 0 ],
								'courier' => $tracking_label[ 0 ],
                                'trackingNumber' => $tracking_details[ 'track_number' ]
							);
							$result          = $helper->setMirakleOrders( 'tracking', $fields_selected );
							$res             = json_decode( $result );
							if ( isset( $res->message ) && $res->message != '' )
