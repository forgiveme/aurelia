...
class Cnc_MarketPlace_Helper_Callapi extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getSingleton('core/session')->getMiraklStoreId();
    }
    ...
    {
        $url = Mage::getStoreConfig('marketplace/configuration/api_url', $this->_getStore());
        $apiKey = Mage::getStoreConfig('marketplace/configuration/api_key', $this->_getStore());
        $shopId = Mage::getStoreConfig('marketplace/configuration/shop_id', $this->_getStore());
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url . $url_between . $import_id . $import_path . "?api_key=" . $apiKey . "&shop_id=" . $shopId .$qstring);
    ...
    public function putCurlResponse($url_api, $url_between, $data_json = '', $offers = '')
    {
        ...
        curl_setopt($ch, CURLOPT_URL, $url . "/api/" . $offers . "/" . $url_between . "?api_key=" . $apiKey . "&shop_id=" . $shopId);
        if (!empty($data_json)) {
    ...
    public function postCurlResponse($url_between, $existing_file, $filename, $type = '')
    {
        ...
        $apiKey = Mage::getStoreConfig('marketplace/configuration/api_key', $this->_getStore());
        $shopId = Mage::getStoreConfig('marketplace/configuration/shop_id', $this->_getStore());
        $request = curl_init($url . $url_between . "?api_key=" . $apiKey . "&shop_id=" . $shopId);
        if ($type == 'json') {
    ...
    public function getCarriers($check = '')
    {
        ...
        $shopId = Mage::getStoreConfig('marketplace/configuration/shop_id', $this->_getStore());
        if ($url && $apiKey) {
            $request = curl_init($url . "/api/shipping/carriers?api_key=" . $apiKey . "&shop_id=" . $shopId);
            curl_setopt($request, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($request, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    public function getShopInfo()
    {
        ...
        if ($url && $apiKey) {
            $request = curl_init($url . "/api/account?api_key=" . $apiKey . "&shop_id=" . $shopId);
            curl_setopt($request, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($request, CURLOPT_SSL_VERIFYPEER, FALSE);
            curl_setopt($request, CURLOPT_SSL_VERIFYHOST, FALSE);
            $res = curl_exec($request);
            if (curl_errno($request)) {
                ...
                $result = json_decode($res);
                $shopId = Mage::getStoreConfig('marketplace/configuration/shop_id', $this->_getStore());
                $shopName = Mage::getStoreConfig('marketplace/configuration/shop_name', $this->_getStore());
    ...
    public function deleteBulkOffersApi($offer_data)
    {
        ...
        $url_api = Mage::getStoreConfig('marketplace/configuration/api_url', $this->_getStore());
    ...
    public function getMirakleDataCurlResponse($offset, $max, $recursive, $date)
    {
        ...
        curl_setopt($ch, CURLOPT_URL, $url . "/api/orders?api_key=" . $apiKey . "&max=" . $max . $date_qstring . $offset_qstring . "&shop_id=" . $shopId);
    ...
    public function executeTrackingOrderStatus($url_api, $fields_selected, $pass_order_id)
    {
        ...
        if ($fields_selected["carriercode"] != 'custom') {
