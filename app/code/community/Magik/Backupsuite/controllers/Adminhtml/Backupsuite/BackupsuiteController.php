			...
			if ($post_data) {
				try {	
					...
					if($checkBackuptype==0) {
						  ...
						  $cmd = "mysqldump -alv -h ".$profileData['db_host']." -u".$profileData['db_username']." -p".str_replace(")","\)",$profileData['db_password'])."  ".$profileData['db_name']." ".$IgnoretableList."2>>". $logDirPath." | gzip > ".$backupDirPath."/".$db_file;
						  system($cmd);
					...
					if($profileData['storage_type']==1) {
						...
						if($profileData['storage_type']==5) {
						    ...
						    if(is_file($filePath)) { 
							if($checkBackuptype==0){
							      $dbID=Mage::getModel('backupsuite/gdriveauth')->uploadToGdrive($db_file);
							      $fileID=Mage::getModel('backupsuite/gdriveauth')->uploadToGdrive($backup_filename);
							    // Mage::getModel('backupsuite/backupsuite')->markArchive(now(), 5, $db_file, $dbID);
							    // Mage::getModel('backupsuite/backupsuite')->markArchive(now(), 5, $backup_filename, $fileID);
							}
							if($checkBackuptype==1){
							      $fileID=Mage::getModel('backupsuite/gdriveauth')->uploadToGdrive($backup_filename);
							      //Mage::getModel('backupsuite/backupsuite')->markArchive(now(), 5, $backup_filename, $fileID);
						...
						if($profileData['delete_enable']!='0') {
						  try{
							...
							foreach($deleteData as $val) {
							      ...
							      if($profileData['storage_type'] == 1) { //ftp
								      ...
								      ftp_delete($conn, $val->getDbfilename());
								      ftp_close($conn);

							      }
							      if($profileData['storage_type'] == 2) { //amazon s3
								      Mage::getModel('backupsuite/amazons3')->removeS3($val->getFilename(), $profileData['amazon_bucket'], $profileData['amazon_access_key'], $profileData['amazon_secret_key']);
								        Mage::getModel('backupsuite/amazons3')->removeS3($val->getDbfilename(), $profileData['amazon_bucket'], $profileData['amazon_access_key'], $profileData['amazon_secret_key']);
							      } 
							      if($profileData['storage_type'] == 3) {
									      Mage::getModel('backupsuite/dropbox')->removeFromDropbox($val->getFilename(),$profile_Id);
									      Mage::getModel('backupsuite/dropbox')->removeFromDropbox($val->getDbfilename(),$profile_Id);			
							      } 
							      if($profileData['storage_type'] == 4) {
								      Mage::getModel('backupsuite/boxnet')->removeFromBoxnet($val->getFilename(),$profile_Id);
								      Mage::getModel('backupsuite/boxnet')->removeFromBoxnet($val->getDbfilename(),$profile_Id);
							      } 
							      if($profileData['storage_type'] == 5) {
