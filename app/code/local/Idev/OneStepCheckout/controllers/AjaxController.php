...
class Idev_OneStepCheckout_AjaxController extends Mage_Core_Controller_Front_Action
{
	...
	$detailsOfCopuons=$oRule->getData();
	$couponRuleId=$detailsOfCopuons['rule_id'];
       ...
       if(Mage::getSingleton('customer/session')->isLoggedIn())
       	{
			...
			$couponDetails=$quote->getCouponCode();
		...
		if(empty($couponDetails))
            ...
            if ($couponCode) {
                if ($couponCode == $quote->getCouponCode()) {
                    ...
                    $response['message'] = $this->__('Success! We hope you enjoy your gift from Aurelia.');
			...
			else
			{
				...
				->save();
                $response['success'] = true;
                $response['message'] = $this->__('Coupon code was cancelled successfully.');
			...
			// if trying to add more then 1 coupon it gives error message 
				$response['success'] = false;
				$response['error'] = true;
				$response['message'] ='Oops! Looks like your code could be invalid or you already have an offer in your basket. Contact <a href="mailTo:customerservices@aureliaskincare.com">customerservices@aureliaskincare.com</a> for help.';
			}
	...
	else
	{
	  ...
	  $response['success'] = false;
          $response['error'] = true;
          $response['message'] = $this->__('Please join Aurelia Loyalty to benefit from offers.');
