...
abstract class Mage_Paypal_Model_Api_Abstract extends Varien_Object
{
   ...
   /**
    ...
    protected function _exportLineItems(array &$request, $i = 0)
    {
		...
		//if($_SERVER['REMOTE_ADDR']=='103.254.98.190') {
			$disAmount = (float) Mage::helper('checkout/cart')->getQuote()->getCustomfeeAmount();
			if($disAmount < 0) {
				$request['L_NAME'.$i] = 'First order discount';
				$request['L_AMT'.$i] = $disAmount;
				$request['L_QTY'.$i] = 1;
			}
		//}
        return $result;
