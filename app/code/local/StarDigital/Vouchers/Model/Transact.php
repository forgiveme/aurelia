...
class Stardigital_Vouchers_Model_Transact extends Mage_Core_Model_Abstract
{
    ...
    public function _construct()
    {
        ...
        $this->_monthlyDealCampaignId = Mage::getStoreConfig('vouchers/monthly_deals/campaign_id');
    ...
    protected function _getOrderItemsHtml($order, $vouchers, $isMonthlyDeal = false)
    {
        ...
        if ($isMonthlyDeal) {
            ...
            $return .= '
                    ...
                    <td valign="top" align="left" style="font-size:12px;font-family:Georgia,\'Times New Roman\',Times,serif;color:#999999;padding:9px 10px;border-bottom:1px dotted #666"></td>
                    <td valign="top" align="center" style="font-size:12px;padding:9px 10px;border-bottom:1px dotted #666;color:#999999;font-family:Georgia,\'Times New Roman\',Times,serif"></td>
                    <td valign="top" align="right" style="font-size:12px;padding:9px 10px;border-bottom:1px dotted #666;color:#999999;font-family:Georgia,\'Times New Roman\',Times,serif">
                        <span></span>
                    </td>
                </tr>
    ...
    protected function _sendNewOrderEmail($order, $vouchers, $campaignId, $isMonthlyDeal = false)
    {
    ...
    <XTMAILING>
        ...
        <RECIPIENT>
            <EMAIL>' . 'info@aureliaskincare.com' . '</EMAIL>
            <BODY_TYPE>HTML</BODY_TYPE>
        ';
    ...
    {
        $billingAddress = $order->getBillingAddress();
        $voucherCode = $vouchers[0];

        $details = array();
        $details['CustomerName'] = $billingAddress->getFirstname() . ' ' . $billingAddress->getLastname();
        $details['ordernumber'] = $order->getIncrementId();
        $details['GIFTCODE'] = $voucherCode['code'];

        $xmlData ='
    <XTMAILING>
        <CAMPAIGN_ID>' . $this->_monthlyDealCampaignId . '</CAMPAIGN_ID>
        <SHOW_ALL_SEND_DETAIL>true</SHOW_ALL_SEND_DETAIL>
        <SEND_AS_BATCH>false</SEND_AS_BATCH>
        <NO_RETRY_ON_FAILURE >false</NO_RETRY_ON_FAILURE>
        <SAVE_COLUMNS>
            <COLUMN_NAME>CustomerName</COLUMN_NAME>
            <COLUMN_NAME>ordernumber</COLUMN_NAME>
            <COLUMN_NAME>GIFTCODE</COLUMN_NAME>
        </SAVE_COLUMNS>
        <RECIPIENT>
            <EMAIL>' . $order->getCustomerEmail() . '</EMAIL>
            <BODY_TYPE>HTML</BODY_TYPE>
    ...
    <XTMAILING>
        ...
        <RECIPIENT>
            <EMAIL>cron.stardigital@gmail.com</EMAIL>
            <BODY_TYPE>HTML</BODY_TYPE>
        ';
