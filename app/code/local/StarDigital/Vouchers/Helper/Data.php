...
class Stardigital_Vouchers_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    protected function _generateVoucherModel($voucherCode, $email, $orderId, $voucherPrice, $voucherFromDate, $voucherToDate, $voucherType, $ruleId, $productId = null, $isGift = 0)
    {
        $voucher = Mage::getModel('vouchers/vouchers');
    ...
    public function generateMonhtlyDealVouchers($order)
    {
        $minOrderAmount = Mage::getStoreConfig('vouchers/monthly_deals/order_total');
        $fromDateTimestamp = strtotime(Mage::getStoreConfig('vouchers/monthly_deals/from_date') . ' 00:00:00');
        $toDateTimestamp = strtotime(Mage::getStoreConfig('vouchers/monthly_deals/to_date') . ' 23:59:59');
        $orderDateTimestamp = strtotime($order->getCreatedAt());
        $ruleId = Mage::getStoreConfig('vouchers/monthly_deals/rule_id');

        if ($order->getGrandTotal() >= $minOrderAmount) {
            if ($orderDateTimestamp >= $fromDateTimestamp && $orderDateTimestamp <= $toDateTimestamp) {
                ...
                Mage::getModel('vouchers/silverpop')->sendMonthlyDealsDetails(
    ...
    public function getOrderVouchers($order, $voucherGeneratedAs = null)
    {
        $vouchers = array();
        ...
        if (!is_null($voucherGeneratedAs)) {
            switch ($voucherGeneratedAs) {
                case 'giftVouchers':
	...
	public function isMonthlyDealVouchersEnabled()
    {
        if (Mage::getStoreConfig('vouchers/monthly_deals/enabled') == '1') {
            return true;
        }
